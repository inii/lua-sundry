...
workSpaceDir = useIDENode ? process.argv[2].replace("--gulpfile=", "").replace("\\.laya\\publish_youkugame.js", "").replace("/.laya/publish_youkugame.js", "") + "/" : "./../";
...
gulp.task("preCreate_youku", function() {
	...
	releaseDir = path.join(workSpaceDir, "release", "youkugame").replace(/\\/g, "/");
	config = pubsetJson[12];
	if (process.platform === "darwin") {
		commandSuffix = "";
	}
	// let copyLibsList = [`${workSpaceDir}/bin/libs/laya.youkumini.js`];
...
gulp.task("copyPlatformFile_youku", ["preCreate_youku"], function() {
	let adapterPath = path.join(ideModuleDir, "../", "out", "layarepublic", "LayaAirProjectPack", "lib", "data", "youkufiles");
...
gulp.task("modifyFile_youku", ["copyPlatformFile_youku"], function() {
	// 修改game.json文件
	...
	conJson.screenOrientation = config.youkuInfo.screenOrientation;
...
gulp.task("version_youku", ["modifyFile_youku"], function () {
	if (config.version) {
...
gulp.task("buildYKMiniProj", ["version_youku"], function() {
