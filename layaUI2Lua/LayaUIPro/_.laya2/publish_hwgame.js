...
workSpaceDir = useIDENode ? process.argv[2].replace("--gulpfile=", "").replace("\\.laya\\publish_hwgame.js", "").replace("/.laya/publish_hwgame.js", "") + "/" : "./../";
...
gulp.task("preCreate_HW", function(cb) {
	...
	releaseDir = path.join(workSpaceDir, "release", "hwgame").replace(/\\/g, "/");
	releaseDir = tempReleaseDir = path.join(releaseDir, "temprelease");
	config = pubsetJson[10]; // 只用到了 config.hwInfo|config.hwSign
    toolkitPath = path.join(ideModuleDir, "../", "out", "layarepublic", "pub_huawei");
...
gulp.task("copyPlatformFile_HW", ["preCreate_HW"], function() {
	let hwAdapterPath = path.join(ideModuleDir, "../", "out", "layarepublic", "LayaAirProjectPack", "lib", "data", "hwfiles");
...
gulp.task("installModules_HW", ["copyPlatformFile_HW"], function() {
...
gulp.task("version_HW", ["modifyFile_HW"], function () {
	if (config.version) {
		let versionPath = projDir + "/version.json";
