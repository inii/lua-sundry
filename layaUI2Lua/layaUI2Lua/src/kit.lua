...
function excute(cmdStr, path1, path2)
    ...
    if path1 then
        ...
        table.insert(tab, path1)
    end

    if path2 then
        path2 = string.gsub(path2, [[/]], [[\]])
        table.insert(tab, path2)
    end
...
function write2File(path, content, mod)
...
function log2file(content, ...)
    ...
    kit.write2File("log.txt", content .. "\n", 'a+')
end

function readFile(path, mod)
    path = string.gsub(path, [[\]], [[/]])
    local file = io.open(path, mod or "r")
...
function kit.write2FileAdd(path, content, mod)
